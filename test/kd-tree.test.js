...
import kdTree, { viewDepthSort } from '../lib/kd-tree.js';
import IndexedPointArray from '../lib/pointarray.js';
...
describe('kdTree', () => {
  it('should create a tree correctly', () => {
    ...
    expect(tree.cleft.cleft.range).to.deep.equal([0, 1]);
    expect(tree.cleft.cright.range).to.deep.equal([2, 3]);
    expect(tree.cright.d).to.equal(1);
    expect(tree.cright.val).to.equal(4.5);
    expect(tree.cright.cleft.range).to.deep.equal([4, 5]);
    expect(tree.cright.cright.range).to.deep.equal([6, 7]);
  });
  ...
  it('should sort the tree by depth correctly', () => {
    ...
    viewDepthSort(tree, {x: 3, y: -3, z: -3});

    expect(tree.cleft.val).to.equal(4.5);
    expect(tree.cleft.cleft.range).to.deep.equal([6, 7]);
    expect(tree.cleft.cright.range).to.deep.equal([4, 5]);
    expect(tree.cright.d).to.equal(1);
    expect(tree.cright.val).to.equal(2.5);
    expect(tree.cright.cleft.range).to.deep.equal([2, 3]);
    expect(tree.cright.cright.range).to.deep.equal([0, 1]);
