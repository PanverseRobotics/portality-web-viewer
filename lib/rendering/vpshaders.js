// permType can be 'kdtree' or 'full'
function createRenderProgram(gl, permType='kdtree') {
    let samplerDefs
    let getTexCoord
    ...
    if (permType == 'full') {
        ...
        ivec2 texCoord = ivec2(inner_idx, outer_idx);
        `        
    } else {
        samplerDefs = `
    ...
    uniform sampler2D covP1Texture;    // Texture containing covP1 data
    uniform sampler2D covP2Texture;    // Texture containing covP2 data
    ...
    vec3 computeCov2D(const vec3 t, const vec2 focal, const vec4 cov3dp1, const vec2 cov3dp2, const mat4 viewmatrix)
    {
        mat3 J = mat3(
        ...
        mat3 Vrk = mat3(
            cov3dp1[0], cov3dp1[3], cov3dp2[0],
            cov3dp1[3], cov3dp1[1], cov3dp2[1],
            cov3dp2[0], cov3dp2[1], cov3dp1[2]);
    ...
    void main() {
        ...
        vec4 covP1 = texelFetch(covP1Texture, texCoord, 0).xyzw;
        vec2 covP2 = texelFetch(covP2Texture, texCoord, 0).xy;
        ...
        vec3 vCov2d = computeCov2D(t.xyz, vec2(focal, focal), covP1, covP2, uView);
