// permType can be 'groups' or 'full'
function createRenderProgram(gl, permType='groups') {
    let samplerDefs
    let getTexCoord
    ...
    if (permType == 'full') {
        ...
        uint idx = texelFetch(perm_idx, ivec2(inner_idx, outer_idx), 0)[0];
        ivec2 texCoord = ivec2(int(idx) % textureSize.x, int(idx) / textureSize.x);
        `        
    } else {
        samplerDefs = `
