...
function viewDollyTranslateRotateTouch(event, lastTouchPosition, viewParams){
    // this one translates focus and eye together with vertical and rotates view with horizontal
    // find the vector from the eye to the focus
    let eyeToFocus = [viewParams.focusPosition[0] - viewParams.eyePosition[0], viewParams.focusPosition[1] - viewParams.eyePosition[1], viewParams.focusPosition[2] - viewParams.eyePosition[2]];
    ...
    let lastTouch1x = lastMousePosition[0];
    let lastTouch1y = lastMousePosition[1];
    let lastTouch2x = lastMousePosition[2];
    let lastTouch2y = lastMousePosition[3];
    ...
    let angleChange = (angle - lastAngle)/viewParams.lookSensitivity;

    

    // translate the focus position by dx * right + dy * eyeToFocus
    viewParams.focusPosition = [viewParams.focusPosition[0] + distanceChange * eyeToFocus[0], viewParams.focusPosition[1]  + distanceChange * eyeToFocus[1], viewParams.focusPosition[2] + distanceChange * eyeToFocus[2]];
    // translate the eye position by dx * right + dy * eyeToFocus
    viewParams.eyePosition = [viewParams.eyePosition[0] + distanceChange * eyeToFocus[0], viewParams.eyePosition[1]  + distanceChange * eyeToFocus[1], viewParams.eyePosition[2] + distanceChange * eyeToFocus[2]];
    ...
    let rightNorm = Math.sqrt(right[0] ** 2 + right[1] ** 2 + right[2] ** 2);
    right = [right[0] / rightNorm, right[1] / rightNorm, right[2] / rightNorm];
    let upNew = [up[0] * Math.cos(angleChange) + right[0] * Math.sin(angleChange), up[1] * Math.cos(angleChange) + right[1] * Math.sin(angleChange), up[2] * Math.cos(angleChange) + right[2] * Math.sin(angleChange)];
    let upNewNorm = Math.sqrt(upNew[0] ** 2 + upNew[1] ** 2 + upNew[2] ** 2);
    upNew = [upNew[0] / upNewNorm, upNew[1] / upNewNorm, upNew[2] / upNewNorm];
    viewParams.up = upNew;
}
...
function viewStrafeVert(event, lastMousePosition, viewParams){
    ...
    // console.log("strafe");
    // console.log(viewParams.focusPosition);
    let dx = -1*3 * (event.clientX - lastMousePosition[0])/viewParams.lookSensitivity;
    let dy = 3 * (event.clientY - lastMousePosition[1])/viewParams.lookSensitivity;
    // console.log(dx);
    // console.log(dy);
    // find the vector from the eye to the focus
...
function viewPan(event, lastMousePosition, viewParams){
    ...
    let dx = -1*event.clientX - lastMousePosition[0];
    let dy = -1*event.clientY - lastMousePosition[1];
...
function viewMoveTouch(event, lastTouchPosition, viewParams){
    ...
    if (event.touches.length == 1) {
        viewOrbit(event.touches[0], lastTouchPosition, viewParams);
    } else if (event.touches.length == 2) {
        
        
        viewDollyTranslateRotateTouch(event, lastTouchPosition, viewParams);
...
function viewDollyWheelTranslate(event, viewParams){
    ...
    // with middle click, strafe up and down
    let dy = -1*event.deltaY / viewParams.lookSensitivity;
...
export { viewMoveMouse, viewDollyWheelTranslate, viewMoveKey, viewMoveTouch, viewAutoSpin, stopAutoSpin };