...
function viewOrbit(delta, viewParams) {
    let r = eulerRotation([-delta[1], -delta[0], 0.0]); // Start with inverse rotation
    let t = [0.0, 0.0, viewParams.radius]; // Focus point coords.
    mat3vecmultiply(t, r, t); // New focus point coordinates.
    let p = [-t[0],-t[1],viewParams.radius-t[2]]; // Difference.
    mat3transpose(r, r);  // Invert rotation back.
    let transformMatrix = buildTransformMatrix(rotation, p);
    return mat4multiplyNew(transformMatrix, viewParams.viewMatrix);
}
