...
function viewOrbit(event, lastMousePosition, viewParams) {
    ...
    // var translation_matrix = buildTransformMatrix(rotation, viewParams.radius*Math.sin(dx)*Math.cos(dy), -viewParams.radius*Math.sin(dy)*Math.cos(dx), 0);
    var translation_matrix = buildTransformMatrix(rotation, viewParams.radius*Math.sin(dx),  viewParams.radius*Math.sin(-dy), 0);
...
function viewDollyTranslateRotateTouch(event, lastMousePosition, viewParams) {
...
function viewRadiusWheel(event, viewParams) {
    viewParams.radius += event.deltaY / viewParams.lookSensitivity;

    // clamp the radius to [0.1, 30]
...
function viewDollyWheel(event, isKeyDown, keyPressed, viewParams) {
// with middle scroll, strafe in and out
    let dy = -event.deltaY / viewParams.lookSensitivity;
    if (isKeyDown) {
        if (keyPressed == "Control") {
            var dRadius = Math.max(0.1, Math.min(30, dy + viewParams.radius));
            viewParams.radius = dRadius;
        } 
    } else {
        let rotation = I(4);
        var translation_matrix = buildTransformMatrix(rotation, 0, 0, dy);
...
// rework this entire thing to use the view matrix when you update for smoothness based on key state based movement
...
export { viewMoveMouse, viewDollyWheel, viewMoveKey, viewMoveTouch, viewAutoSpin, stopAutoSpin, getRadius, lazyInitializeViewMatrix };
