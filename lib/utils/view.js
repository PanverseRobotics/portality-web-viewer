...
function viewMoveTouch(event, lastMousePosition, viewParams){
    // copilot generated code below, BEWARE
    if (event.touches.length == 1) {
        viewOrbit(event.touches[0], lastMousePosition, viewParams);
    } else if (event.touches.length == 2) {
        let dx = event.touches[0].clientX - lastMousePosition[0];
        let dy = event.touches[0].clientY - lastMousePosition[1];
        viewParams.radius = getRadius(viewParams);
        viewParams.radius += dy / viewParams.lookSensitivity;
