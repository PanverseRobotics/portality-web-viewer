...
function updateViewMatrix(viewParams, angles, p) {
    ...
    // Compute new predicted matrix as the transform matrix applied to viewParams.matrix n times,
    // where n is the number of frames in the future we want to predict.
    // n = viewParams.sortInterval.
    let predictedMatrix = new Float32Array(16);
    mat4multiply(predictedMatrix, transformMatrix, viewParams.matrix);
    for (let i = 0; i < (viewParams.sortInterval-1); i++) {
        mat4multiply(predictedMatrix, transformMatrix, predictedMatrix);
    }     
    newViewParams.predictedMatrix = predictedMatrix;
...
function viewOrbit(delta, viewParams) {
    ...
    let n = viewParams.sortInterval;
    let s1 = eulerRotation([delta[1], delta[0], 0.0]);
    let sn = eulerRotation([n*delta[1], n*delta[0], 0.0]);
    let transformMatrix1 = buildTransformMatrix(s1, [0, 0, 0]);
    let transformMatrixn = buildTransformMatrix(sn, [0, 0, 0]);
    let newViewMatrix1 = mat4multiplyNew(transformMatrix1, viewParams.matrix);
    let newViewMatrixn = mat4multiplyNew(transformMatrixn, viewParams.matrix);

    // And then move the camera back to the original focus point,
    // Via t2 = -R*t + [0, 0, -viewParams.radius]
    submat3(r, newViewMatrix1);
    mat3vecmultiply(p, r, t);
    let t21 = [-p[0],-p[1],-p[2]-viewParams.radius];
    newViewMatrix1[12] = t21[0];
    newViewMatrix1[13] = t21[1];
    newViewMatrix1[14] = t21[2];
    newViewParams.matrix = newViewMatrix1;

    submat3(r, newViewMatrixn);
    mat3vecmultiply(p, r, t);
    let t2n = [-p[0],-p[1],-p[2]-viewParams.radius];
    newViewMatrixn[12] = t2n[0];
    newViewMatrixn[13] = t2n[1];
    newViewMatrixn[14] = t2n[2];
    newViewParams.predictedMatrix = newViewMatrixn;
