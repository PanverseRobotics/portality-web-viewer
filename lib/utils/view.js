...
function viewDollyTranslateRotate(delta, viewParams) {
    let newViewParams = updateViewMatrix(viewParams, [0, 0, delta[0]], [0, 0, -2*delta[1]]);
    newViewParams.radius = viewParams.radius - 2*delta[1];
    return newViewParams;
}

function viewDolly(posDelta, viewParams) {
    let newViewParams = updateViewMatrix(viewParams, [0, 0, 0], [0, 0, -posDelta]);
    newViewParams.radius = viewParams.radius + posDelta;
    return newViewParams
}
