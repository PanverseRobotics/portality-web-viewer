function createBitonicKernel(gl, n, m) {
    const fragSrc =
    `#version 300 es
    ...
    uniform lowp sampler2D bitonic_user_x;
    ...
    void main(void) {
        ...
        float xl = texelFetch(bitonic_user_x, ivec2(int(l), fragCoord.y), 0)[0];
        float xi = texelFetch(bitonic_user_x, fragCoord, 0)[0];
    ...
    const innerKernel = function (x, sort_step_j, sort_step_k) {
        ...
        const glVariable95 = gl.getUniformLocation(shdr.program, 'bitonic_user_x');
