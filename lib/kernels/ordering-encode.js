function encodeKernel(gl, n, m) {
    ...
    uniform uint user_groupu_umask;
    float kernelResult;
    out vec4 data0;

    void main(void) {
        ivec2 fragCoord = ivec2(gl_FragCoord.xy);
        float val = texelFetch(user_x, fragCoord, 0)[0];
        uint user_floatu_umask = ~user_groupu_umask;
    ...
    const innerKernel = function (x) {
        ...
        const glVariable17 = gl.getUniformLocation(shdr.program, 'user_groupu_umask');
