...
function createDepthIndexedKernel(gl, n, m) {
    const fragSrc =
    `#version 300 es
    precision highp float;
    precision highp int;
    ...
    void main(void) {
        ivec2 fragCoord = ivec2(gl_FragCoord.xy);
        vec3 position_dk = texelFetch(uCoords, fragCoord, 0).xyz;
        ivec2 texSize = textureSize(uCoords, 0);

        uint idx = uint(fragCoord.x) + uint(fragCoord.y)*uint(texSize.x);

        vec4 clipPosition = uViewProj_dk * vec4(position_dk, 1.0);

        data = uvec2(floatBitsToUint(clipPosition.z / clipPosition.w), idx);
    }`
...
export { createDepthKernel, createDepthIndexedKernel };